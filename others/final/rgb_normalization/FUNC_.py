...
def average_profile(theta_start, theta_end, delta_theta, xc, yc, s, img):
    ...
    img_profile = np.zeros(s)

    for theta in np.arange(theta_start, theta_end+delta_theta, delta_theta):
        img_profile = img_profile + image_profile(img, xc, yc, theta, s)

    img_profile = img_profile/(int((theta_end+delta_theta-theta_start)/delta_theta)-1)
